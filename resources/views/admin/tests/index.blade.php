              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach($tests as $test)
                  <tr>
                    <td>{{$test->id}}</td>
                    <td>{{$test->title}}</td>
                    <td>{{$test->getEventTitle()}}</td>
                    <td>
                      <img src="{{$test->getImage()}}" alt="" width="100">
                    </td>
                    <td><a href="{{route('tests.edit', $test->id)}}" class="fa fa-pencil"></a>
                    {!! Form::open(['route' => ['tests.destroy', $test->id], 'method' => 'delete' ])!!}
                    <button onclick="return confirm('Вы действительно хотите удалить этот тест?')" type="submit" class="delete"> 
